...
@section('content')
    <div class="w-full">
        <div class="mb-5">
            <h2 class="text-lg font-semibold text-neutral-900 dark:text-white">Create Role</h2>
        </div>
        <div class="card p-5">
            <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input id="name" name="name" type="text" class="input @error('name') input-error @enderror" value="{{ old('name') }}" placeholder="e.g. editor" required>
                                ...
                                <div class="flex flex-wrap gap-3">
                                    @foreach($perms as $perm)
                                        <label class="inline-flex items-center gap-1.5">
                                            <input type="checkbox" name="permissions[]" value="{{ $perm->name }}" @checked(in_array($perm->name, old('permissions', [])))>
